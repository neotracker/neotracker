...
function portToUrlString(options) {
  return options.skipDefaultPort && options.port === 443
    ? ''
    : ':' + options.port;
}
...
export default function enforceHTTPS(options) {
  ...
  return (ctx, next) => {
    ...
    if (
      options.trustProtoHeader &&
      ctx.request.header['x-forwarded-proto'] != null
    ) {
    ...
    if (
      !secure &&
      options.trustAzureHeader &&
      ctx.request.header['x-arr-ssl']
    ) {
      secure = true;
    }
