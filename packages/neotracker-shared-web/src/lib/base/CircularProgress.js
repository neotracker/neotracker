...
const styles = (theme: any) => {
  ...
  return {
    ...
    indeterminateCircle: {
      ...
      strokeDashoffset: '0%',
      animation: `mui-scale-progress-circle 1300ms ${
    ...
    '@keyframes mui-scale-progress-circle': {
      ...
      '50%, 58%': {
        strokeDasharray: `${calcFallback(65, TMP_SIZE)}, ${calcFallback(
          100,
          TMP_SIZE,
        )}`,
...
function CircularProgress(props) {
  ...
  if (mode === 'determinate') {
    ...
    circleStyle.strokeDasharray =
      // eslint-disable-next-line
      (calcFallback(100, TMP_SIZE) * relVal).toString() +
      ',' +
      calcFallback(100, TMP_SIZE).toString();
