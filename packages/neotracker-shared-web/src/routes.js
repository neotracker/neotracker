...
export const makeTransaction = (hash: string = TRANSACTION_HASH_PARAM) =>
  `/tx/${hash}`;
...
export const isTransaction = (path: string) =>
  path.startsWith('/tx/') || path.startsWith('/browse/tx/');

// Address
export const ADDRESS_HASH_ARGUMENT = 'addressHash';
const ADDRESS_HASH_PARAM = `:${ADDRESS_HASH_ARGUMENT}`;
export const makeAddress = (hash: string = ADDRESS_HASH_PARAM) =>
  `/address/${hash}`;
...
export const isAsset = (path: string) =>
  path.startsWith('/asset/') || path.startsWith('/browse/asset/');

// Contract
export const CONTRACT_HASH_ARGUMENT = 'contractHash';
const CONTRACT_HASH_PARAM = `:${CONTRACT_HASH_ARGUMENT}`;
export const makeContract = (hash: string = CONTRACT_HASH_PARAM) =>
  `/contract/${hash}`;
...
export const isContract = (path: string) =>
  path.startsWith('/contract/') || path.startsWith('/browse/contract/');
