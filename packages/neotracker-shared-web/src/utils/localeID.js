...
export default function parse(locale) {
  ...
  const keyword =
    keywordPos !== -1 ? stringLocale.substr(keywordPos + 1) : void 0;

  const localeWithoutKeyword =
    keywordPos !== -1 ? stringLocale.substr(0, keywordPos) : stringLocale;
  ...
  if (script) {
    retVar.script = _.capitalize(script.toLowerCase());
  }
...
export function getBest(supported, locale, defaultLocale, getAnyCountry) {
  const lgs = _.isArray(supported) ? prepareSupported(supported) : supported;
